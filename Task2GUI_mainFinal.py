...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.actionReconstruct.triggered.connect(lambda: self.reConstruct(self.numSamples, self.samplingInterval, self.amp1, self.time1))
    ...
    def reConstruct(self, numSample, tVal, ampArr, tSample):
        ...
        ampArr = np.array(ampArr, dtype=float)
        T = 1.0 / self.Fsample
        t = np.array(self.time1)

        # Whittaker-Shannon: sum over actual sample points with their amplitudes
        sincMatrix = np.sinc((t[:, np.newaxis] - tSample[np.newaxis, :]) / T)
        sumSignalReconstruct = sincMatrix @ ampArr  # shape: (len(t),)

        self.secindaryChannel.clear()
        self.secindaryChannel.plot(t, sumSignalReconstruct, pen=pyqtgraph.mkPen("#00cc66", width=2))
        self.updateReconstruct(list(t), list(sumSignalReconstruct), self.freqSlider.value())
    ...
    def updateReconstruct(self, time, sum, val):
        ...
        if val == 1:
            self.array1 = np.array(sum)
        elif val == 2:
            self.array2 = np.array(sum)
        elif val == 3:
            self.array3 = np.array(sum)

        # Only plot arrays that have been filled (not the default 0)
        if isinstance(self.array1, np.ndarray):
            pen = pyqtgraph.mkPen("#ffaa00" if val == 1 else "#444444", width=2)
            self.secindaryChannel.plot(time, self.array1[:len(time)], pen=pen)
        if isinstance(self.array2, np.ndarray):
            pen = pyqtgraph.mkPen("#ffaa00" if val == 2 else "#444444", width=2)
            self.secindaryChannel.plot(time, self.array2[:len(time)], pen=pen)
        if isinstance(self.array3, np.ndarray):
            pen = pyqtgraph.mkPen("#ffaa00" if val == 3 else "#444444", width=2)
            self.secindaryChannel.plot(time, self.array3[:len(time)], pen=pen)
