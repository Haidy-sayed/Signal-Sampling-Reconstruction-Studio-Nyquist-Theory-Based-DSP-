...
class Ui_Form(object):
    def setupUi(self, Form):
        ...
        self.pushButton_5 = QtWidgets.QPushButton(self.composer_control)
        ...
        # FIX: removed duplicate connection here — connection is set once in retranslateUi
    ...
    def retranslateUi(self, Form):
        ...
        # FIX: single connection that does both — add entry to combobox AND store the signal data
        self.pushButton_5.clicked.connect(lambda: (self.addSinCombo(), self.add_to_summation()))
    ...
    def read(self):
        ...
        try:
            ...
            print("phase should be a number")
    ...
    def add_to_summation(self):
        ...
        # FIX: guard against adding before any signal is drawn
        if value is None:
            print("No signal drawn yet — click 'Draw sin' first")
            return
        key = "signal{}".format(self.comboBox.count() - 1)
        self.summation_dict[key] = value

    def plot_summation(self):
        self.curve2.clear()
        # FIX: safe summation that doesn't crash on empty dict
        if not self.summation_dict:
            print("No signals added to summation yet")
            return
        values = list(self.summation_dict.values())
        self.sum_amp = values[0].copy()
        for v in values[1:]:
            self.sum_amp += v
        try:
    ...
    def gen_csv(self):
        # FIX: guard against saving before anything is composed
        if len(self.sum_amp) != len(self.t):
            print("Nothing composed yet — compose a signal first")
            return
